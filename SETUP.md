...
## 8. Installing it as an app (PWA)

The app is installable — once it's served over `https://` (or `localhost`),
visiting it in Chrome/Edge on desktop or Android shows an install prompt
(or an "Install app" button in the sidebar/login screen). Installed, it
opens full-screen with its own icon, no browser chrome.

- **Android / desktop Chrome**: the "Install app" button appears
  automatically once the browser decides the site qualifies (valid
  manifest + registered service worker + served over HTTPS). This won't
  appear on `file://` or plain `http://` — it needs Live Server's
  `http://localhost` or a real HTTPS deployment.
- **iOS Safari**: Apple doesn't support the automatic install prompt.
  The app instead shows a hint: tap the Share icon → "Add to Home
  Screen." This is Apple's standard install flow for any web app.
- The service worker (`sw.js`) only caches the app's own HTML/CSS/JS
  files for offline loading — it deliberately never touches Firebase/
  Firestore traffic, so your live attendance data always goes straight
  to the network and is never served stale.
